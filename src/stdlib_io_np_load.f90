...
submodule(stdlib_io_np) stdlib_io_np_load
    ...
    use stdlib_io_zip, only: unzip, unzipped_folder
...
contains
    ...
    module subroutine load_npz_to_arrays(filename, arrays, iostat, iomsg)
        ...
        call list_files(unzipped_folder, files, stat, msg)

        ! call load_unzipped_files_to_arrays(unzipped_bundle, arrays, stat, msg)
    end
    ...
    subroutine list_files(dir, files, stat, msg)
        ...
        ! call execute_command_line('ls temp > file_list.txt', status)
        ! if (status /= 0) then
    ...
    subroutine load_unzipped_files_to_arrays(arrays, stat, msg)
        type(t_array_wrapper), allocatable, intent(out) :: arrays(:)
        integer, intent(out) :: stat
        character(len=:), allocatable, intent(out) :: msg
