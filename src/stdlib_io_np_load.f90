...
contains
    ...
    subroutine load_unzipped_files_to_arrays(files, dir, arrays, stat, msg)
        ...
        do i = 1, size(files)
            open(newunit=io, file=dir//char(files(i)), form='unformatted', access='stream', iostat=stat, iomsg=msg)
            if (stat /= 0) return

            call get_descriptor(io, char(files(i)), this_type, vshape, stat, msg)
            if (stat /= 0) then
