    ...
    module subroutine load_npz_to_arrays(filename, arrays, iostat, iomsg)
        ...
        if (stat /= 0) then
            if (present(iostat)) then
                iostat = stat
                if (allocated(msg)) then
                    call error_stop("Failed to unzip file '"//filename//"'"//nl//msg)
                else
                    call error_stop("Failed to unzip file '"//filename//"'")
                end if
            end if
            return
        end if
        ...
        if (stat /= 0) then
            if (present(iostat)) then
                iostat = stat
                if (allocated(msg)) then
                    call error_stop("Failed to list contents of directory '"//unzipped_folder//"'"//nl//msg)
                else
                    call error_stop("Failed to list contents of directory '"//unzipped_folder//"'")
                end if
            end if
            return
        end if
        ...
        if (stat /= 0) then
            if (present(iostat)) then
                iostat = stat
                if (allocated(msg)) then
                    call error_stop("Failed to load arrays from file '"//filename//"'"//nl//msg)
                else
                    call error_stop("Failed to load arrays from file '"//filename//"'")
                end if
            end if
            return
        end if
